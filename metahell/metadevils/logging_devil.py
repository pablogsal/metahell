...
class LoggingClass(type):
    ...
    def __new__(meta, name, bases, attrs):
        print('Doing evil stuff with {} calling from {}'.format(name,attrs['__module__']))
        for attrname, attr in attrs.items():
            if isfunction(attr) :
                attrs[attrname] = _logging_method(name,attr)

        # If the class has __slots__ we need to include the logger as a possible
        # attribute.
        if '__slots__' in attrs:
            attrs['__slots__'] += ('logger',)
